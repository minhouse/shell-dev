...
INFILE="$MAINDIR/FailedAttemptsactive.csv"
...
printreport(){
	...
	if [ "${data_array[5]}" != async ];then
		if [ "${data_array[5]}" = "" ];then
			data_array[5]="unknown" 
		fi
	fi
	#data_array[5]=`echo ${data_array[5]} |sed "s/\"//g"`
	data_array[5]=`echo ${data_array[5]} |sed "s/^\(.*\)$/\"\1\"/"`
	if [[ ${agent_group[${data_array[10]}]} =~ $agent_group_name ]];then
		gethost ${data_array[10]}
		printf "%-13s%-11s%-25s%-19s%-18s%15s\n" ${data_array[0]} ${data_array[1]} ${data_array[3]} ${data_array[5]} ${data_array[10]} ${hostlist[{${data_array[10]}}
		
		echo -ne i"${data_array[0]} ${data_array[1]},${data_array[3]},${data_array[5]},${data_array[10]},${agent_group[{${data_array[10]}},deny\n" >> $SAD_OUT
	else
...
inputlog(){
	...
	OUT="$MAINDIR/Faild_Attempts/${agent_group_name}-faild.out"
